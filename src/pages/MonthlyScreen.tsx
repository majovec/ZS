...
import { ZSMonthlyIncome, ZSMonthlyFixedExpenses, ZSMonthlyVariableExpenses, ZSMonthlyUnexpectedExpenses } from '@/models/types'

export const MonthlyScreen: React.FC = () => {
  const zsCurrentMonth = useAppStore((state) => state.zsCurrentMonth)
  const setZsCurrentMonth = useAppStore((state) => state.setZsCurrentMonth)
  ...
  const zsMonthlyUnexpectedExpenses = useAppStore((state) => state.zsMonthlyUnexpectedExpenses)
  const setZsMonthlyUnexpectedExpenses = useAppStore((state) => state.setZsMonthlyUnexpectedExpenses)
  ...
  }, [zsMonthlyIncome, zsMonthlyFixedExpenses, zsMonthlyVariableExpenses, zsMonthlyUnexpectedExpenses, calculateZsMonthlySummary])

  const handleIncomeChange = (key: keyof typeof zsMonthlyIncome.items, type: 'plánované' | 'skutečné', value: number) => {
    if (!zsMonthlyIncome) return
    const updated = { ...zsMonthlyIncome }
    updated.items[key][type] = value
    setZsMonthlyIncome(updated)
  }

  const handleFixedChange = (key: keyof typeof zsMonthlyFixedExpenses.items, type: 'plánované' | 'skutečné', value: number) => {
  ...
  const handleVariableChange = (key: keyof typeof zsMonthlyVariableExpenses.items, type: 'plánované' | 'skutečné', value: number) => {
  ...
  return (
    ...
    >
      <div style={{ marginBottom: spacing.lg }}>
        ...
        <p style={{ color: colors.textSecondary, margin: 0 }}>Měsíční přehled - {zsCurrentMonth}</p>
      </div>
